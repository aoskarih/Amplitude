...
float wave(in float x, in float y) {
    ...
    float val = 1 + sin(10*(l+time/10))/(l+1.0);
    return val;
}

float circle_wave(in float x, in float y) {
    float dx = aPos.x - x + center.x - mod(center.x, 0.05);
    float dy = aPos.y - y + center.y - mod(center.y, 4.0/45.0);
...
float point(in float x, in float y) {
    ...
    if (r > 0.1) {
        ...
        float r2 = r*r;
        return 2*(1.0 - 100.0*r2 + 50.0*r2*r2);
    }
}
...
void main() {
    ...
    for (int i = 0; i < 3*128; i=i+3) {
        ...
        if (int(active_f[i]) == 2) {
            new_z = new_z + wave(active_f[i+1], active_f[i+2]);
        } else if (int(active_f[i]) == 3) { 
